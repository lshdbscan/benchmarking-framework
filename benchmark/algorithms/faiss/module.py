...
# Contributed by Viktor Bello Thomsen

def contains_ele(elements, value):
    idx = bisect_left(elements, value)
    if idx >= len(elements):
        return False
...
class FaissDBSCAN(BaseDBSCAN):
    ...
    def cluster(self, X: np.array, epsilon: float, minPts:int):
        ...
        index.add(X)
        epsilon *= epsilon
        D, I = index.search(X, minPts + 1) # all-to-all NN search
        #self.distance = D
        #print(D)
        isCore = D[:,minPts-1] <= epsilon
        #CPs = np.array(sorted(I[:,0][isCore]))
        CPs = np.array([i for i, e in enumerate(isCore) if e])
        print(CPs[:20])
        print(len(CPs))
        self.core_point_indices = CPs
        CP_data = X[CPs]
        CP_index = faiss.IndexFlatL2(d)
        print("CP index trained: ", index.is_trained, flush=True)
        CP_index.add(CP_data)
        print("data added and we have: ", CP_index.ntotal, " Core points", end="\n\n", flush=True)

        start = time.time()
        # #this goes from CP index to full data index.
        CP_I_translator = {order:value for order, value in enumerate(CPs)} #CPs are sorted

        RoundsLeft = 1 << 60
        NN_size = min(len(CPs), minPts)

        print("Merging clusters", flush=True)
        ...
        while(RoundsLeft):
            ...
            if NN_size > len(CPs):
                NN_size = len(CPs)
                RoundsLeft = 1
            print(f"Checking {NN_size}")
            CP_D, CP_I = CP_index.search(CP_data, NN_size)
                
            for j, corepoint in enumerate(CP_data):
                NNS = CP_I[j]
                #idx = NNS[0]
                #print(j)
                to_consider = np.linalg.norm(corepoint - X[CPs[NNS]], axis=1)**2 <= epsilon
                start = 1 if minPts == NN_size else NN_size // 4
                for NN in NNS[to_consider][start:]:
                    if contains_ele(CPs, CP_I_translator[NN]):
                        uf.union(CP_I_translator[j],CP_I_translator[NN])

            if min(CP_D[:, NN_size-1]) >= epsilon or len(set([uf.find(x) for x in CPs])) == 1:
                RoundsLeft = 0
                continue
            #print(NN_size, min(CP_D[:, NN_size-1]), len(set([uf.find(x) for x in CPs])))
            notDone = CP_D[:,NN_size-1] <= epsilon
            CP_data = CP_data[notDone]

            print(f"{len(CP_data)} CPs left")
            RoundsLeft -= 1
        ...
        # #Create Union Find structure
        # uf = UnionFind(range(n))
        # print("Starting on UF step 1", flush=True)
        # #First Do all to all amongst CP
        # for index_p, p in enumerate(CPs):
        #     for index_q, q in enumerate(CPs[index_p+1:]):
        #         dist = np.linalg.norm(data[p]-data[q])
        #         if dist <= eps:
        #             uf.union(p, q)

        print("Time elapsed: ", (time.time()-start) / 1e9, end="\n\n", flush=True)
        print("Labeling border points", flush=True)
        #Second check the minPts NN and see if any of them are CP (break early so that a point is only in 1 cluster)
        #this works since anything that is not a CP now, will have seen atleast epsilon distance points.

        self.border_points = set()
        for index_p, p in enumerate(X):
            if contains_ele(CPs,index_p):
                continue
            for neighbor_idx in I[index_p]:
                if contains_ele(CPs, neighbor_idx) and (0 < np.linalg.norm(p-X[neighbor_idx])**2 <= epsilon):
                    uf.union(index_p, neighbor_idx)
                    self.border_points.add(index_p)
                    break

        print("Time elapsed: ", (time.time()-start) / 1e9, end="\n\n", flush=True)    
        #print("Writing labels", flush=True)
        #print(f"n = {len(X)}: Found {len(self.border_points)} border points, {len(CPs)} core points, and {len(set(range(len(X))) - set(CPs) - self.border_points)} noise points.")

        self.labels = np.array([-1 if uf.find(idx) == idx and (not contains_ele(CPs, idx)) else uf.find(idx) for idx in range(n)])


    def retrieve_labels(self):
        print(self.core_point_indices)
        return self.labels, self.core_point_indices, list(self.border_points)

    def __str__(self):
        return f"FAISSDBSCAN()"
